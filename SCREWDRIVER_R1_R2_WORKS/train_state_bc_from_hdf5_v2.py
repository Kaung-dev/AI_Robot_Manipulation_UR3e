...
parser.add_argument("--out", default="checkpoints/policy_state_bc_mimic_v2.pth")
...
def load_dataset(hdf5_path: str):
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        for demo_key in demos:
            ...
            # v2 obs-alignment: dims 21:28 (target_object_position) are
            # generated_commands("object_pose") — a resampling GOAL = noise w.r.t.
            # the action, and a frame/semantic mismatch with eval. Replace with the
            # actual object position (robot-root, == dims 18:21) + identity quat so
            # the 7 dims carry real signal AND match eval_state_bc_v2.py exactly.
...
if __name__ == "__main__":
    ...
    # v2 observation normalization: standardize each obs dim to mean 0 / std 1.
    # Biggest precision win for an MLP trained on raw world/joint coordinates —
    # tighter approach => better grasp. Stats are SAVED and applied identically
    # at eval (eval_state_bc_v2.py loads obs_mean/obs_std).
    obs_mean = obs_np.mean(axis=0).astype(np.float32)
    obs_std  = obs_np.std(axis=0).astype(np.float32)
    obs_std[obs_std < 1e-6] = 1.0   # guard constant dims (identity quat, phase, etc.)
    ...
    torch.save({
        ...
        "obs_mean": obs_mean,   # v2: normalization stats (eval must apply these)
        "obs_std": obs_std,
        "obs_aligned": True,    # v2: dims 21:28 = object_position + identity quat
    }, out_path)
