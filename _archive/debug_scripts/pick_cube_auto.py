"""Auto-pick the cube with the UR3e + RG2.

Drives env `Isaac-Lift-Cube-UR3e-RG2-IK-Abs-v0` with a warp-based finite-state
...
Franka version are only in:
  - task ID (Isaac-Lift-Cube-UR3e-RG2-IK-Abs-v0)
...
Usage:
    /home/user/IsaacLab/isaaclab.sh -p scripts/pick_cube_auto.py --num_envs 1
    /home/user/IsaacLab/isaaclab.sh -p scripts/pick_cube_auto.py --num_envs 32 --headless
"""
import argparse

from isaaclab.app import AppLauncher

parser = argparse.ArgumentParser(description="Auto-pick the cube with the UR3e + RG2.")
...
TASK = "Isaac-Lift-Cube-UR3e-RG2-IK-Abs-v0"
...
def main() -> None:
    ...
    # demo behaviour.
    print("[pick_cube_auto] before gym.make ...", flush=True)
    env = gym.make(TASK, cfg=env_cfg)
    print("[pick_cube_auto] before env.reset ...", flush=True)
    env.reset()
    print("[pick_cube_auto] env.reset done", flush=True)
    ...
    print(f"[pick_cube_auto] orientation diagnostics written to {diag_path}", flush=True)
