...
class OpenXRDevice(DeviceBase):
    ...
    * "RESET": Reset the tracking and signal simulation reset
    ...
    def get_hand_poses(self, hand: "DeviceBase.TrackingTarget") -> dict[str, np.ndarray]:
        """Return the latest joint poses for the given hand (updated each advance() call)."""
        if hand == DeviceBase.TrackingTarget.HAND_LEFT:
            return self._previous_joint_poses_left
        elif hand == DeviceBase.TrackingTarget.HAND_RIGHT:
            return self._previous_joint_poses_right
        return {}
