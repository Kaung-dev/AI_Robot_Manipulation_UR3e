...
def run_simulation_loop(
    ...
    running_recording_instance = False  # starts paused — enable with open palm gesture
        ...
        while simulation_app.is_running():
            ...
            if gesture_detector is not None and hasattr(teleop_interface, "get_hand_poses"):
                ...
                if gestures["pinch"]:
                    ...
                    env.recorder_manager.export_episodes([0])
                    should_reset_recording_instance = True
                    print("Episode accepted (pinch gesture)")

                if gestures["fist"]:
                    # Discard: skip export and reset
                    env.recorder_manager.record_pre_reset([0], force_export_or_skip=True)
                    should_reset_recording_instance = True
                    print("Episode discarded (fist gesture)")

                if vr_hand_marker is not None:
                    wrist = left_poses.get("wrist")
                    if wrist is not None:
                        vr_hand_marker.update(wrist[:3], running_recording_instance)
