...
# Enable the AR XR profile so the OpenXR session is created and the headset starts streaming.
# Without this call the profile is loaded but never activated — equivalent to clicking "Start VR" in the GUI.
if app_launcher_args.get("xr"):
    try:
        import carb
        from omni.kit.xr.core import XRCore
        s = carb.settings.get_settings()
        # Rasterized rendering: RTX ray tracing gives ~0.4 FPS, causing XR session loss.
        s.set_string("/rtx/rendermode", "RasterizedLighting")
        # Disable post-processing passes that don't contribute to teleoperation quality.
        ...
        # Shadow maps add meaningful cost in rasterized mode; skip for teleoperation.
        s.set_bool("/rtx/shadows/enabled", False)
        # The XR kit file disables texture streaming to avoid rendering artifacts, but on
        # laptops with limited VRAM this causes VK_ERROR_OUT_OF_DEVICE_MEMORY at scene load.
        # Re-enable streaming so textures load on demand instead of all-at-once.
        ...
        print(f"[INFO] XR profiles available: {XRCore.get_singleton().get_profile_name_list()}")
        print(f"[INFO] /xr/profile/ar/enabled = {s.get('/xr/profile/ar/enabled')}")
...
def main() -> None:
    ...
    # Relative control is safe to start active: EE only moves when the hand moves,
    # so the robot won't jump on activation. User keeps hand still until ready.
    teleoperation_active = True
    ...
    # VR camera screens (only created when running the visuomotor task with XR)
    vr_screens = None
    if args_cli.xr:
