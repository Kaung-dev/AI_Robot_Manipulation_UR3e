...
parser.add_argument("--drop_assist", action="store_true",
                    help="Use GraspAssistWrapperV3 instead of V2: adds a policy-gated, reward-shaped RELEASE "
                         "over the basket (anti-hover penalty + release bonus + clean drop). Fixes the V2 failure "
                         "where the policy carries the object in and holds it forever instead of dropping. "
                         "Requires --grasp_assist. Default off => V2 (unchanged, reproducible).")
parser.add_argument("--rebalance_rewards", action="store_true",
                    help="Reward rebalance for the B2-attach contract: the default target_in_hand=+30 pays the "
                         "policy to HOLD (B2 guarantees the hold), so it hovers and never drops. Cut in_hand, "
                         "bump in_basket / release so DELIVERY dominates.")
...
def main():
    ...
    # Strip the per-tool debug-vis ring markers. They target tool-specific prim paths
    # (e.g. "Object/brush_ring", "ToolPliers/pliers_ring") that don't exist in the
    # non-brush scenes -> the FrameTransformer fails to create / debug-vis crashes.
    # These are visualization only (PPO + grasp-assist use scene["ee_frame"] + object).
    # KEEP basket_frame (basket exists everywhere; a reward may use it) and ee_frame
    # (the grasp-assist wrapper needs it).
    _stripped = []
    for _mk in ("brush_frame", "pliers_frame", "scissors_frame", "screwdriver_frame", "ee_tcp_marker"):
        if hasattr(env_cfg.scene, _mk) and getattr(env_cfg.scene, _mk) is not None:
            setattr(env_cfg.scene, _mk, None)
            _stripped.append(_mk)
    print(f"[bc->ppo] stripped per-tool ring markers: {_stripped}", flush=True)

    # Reward rebalance for the B2-attach contract. Default target_in_hand=+30 is the
    # biggest reward and is paid EVERY step the object is held — but B2 guarantees the
    # hold, so the policy maximises return by hovering with the object and NEVER drops
    # (releasing loses the +30). Cut in_hand, make DELIVERY dominate.
    if args_cli.rebalance_rewards:
        R = env_cfg.rewards
        _rb = {"target_in_hand": 5.0, "target_in_basket": 60.0, "release_above_basket": 25.0,
               "target_to_basket": 4.0}
        for _name, _w in _rb.items():
            if hasattr(R, _name) and getattr(R, _name) is not None:
                getattr(R, _name).weight = _w
        print(f"[bc->ppo] REBALANCED rewards: {_rb}", flush=True)
    ...
    if args_cli.grasp_assist:
        ...
        _sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))  # find local v2/v3 wrapper
        if args_cli.drop_assist:
            from grasp_assist_wrapper_v3 import GraspAssistWrapperV3 as GraspAssistWrapper
            tag = "V3 (B2 attach + policy-gated drop)"
        else:
            from grasp_assist_wrapper_v2 import GraspAssistWrapperV2 as GraspAssistWrapper
            tag = "V2 (B2 kinematic attach)"
        env = GraspAssistWrapper(env, target_key=args_cli.grasp_assist_target_key)
        print(f"[bc->ppo] grasp-assist {tag} ENABLED — "
              f"target_key={args_cli.grasp_assist_target_key}, "
        ...
        if args_cli.drop_assist:
            print(f"[bc->ppo]   drop zone: R_DROP={GraspAssistWrapper.R_DROP}m "
                  f"z[{GraspAssistWrapper.Z_MIN},{GraspAssistWrapper.Z_MAX}]  "
                  f"HOLD_PENALTY={GraspAssistWrapper.HOLD_PENALTY}/step  "
                  f"RELEASE_BONUS={GraspAssistWrapper.RELEASE_BONUS}", flush=True)
