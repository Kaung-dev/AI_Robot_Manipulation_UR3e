...
parser.add_argument("--out", default="checkpoints/policy_state_bc_brush_nonorm.pth")
...
if __name__ == "__main__":
    ...
    # NO-NORM build: train on RAW obs (no standardization). This is a PPO WARM-START
    # source — the rsl_rl actor receives raw / empirically-normalized obs, so a
    # normalized BC would mismatch. Obs-alignment (dims 21:28 = object pose) is kept.
    print("[no-norm] training on raw obs (no standardization) — for PPO warm-start")
    ...
    torch.save({
        ...
        "obs_mean": None,       # no-norm: raw obs (no stats)
        "obs_std": None,
        "obs_aligned": True,    # dims 21:28 = object_position + identity quat
    }, out_path)
