...
parser.add_argument("--out", default="eval_results/ppo_v2.json")
...
from grasp_assist_wrapper_v2 import GraspAssistWrapperV2
...
def main():
    ...
    print(f"[eval-ppo-v2] disabled terminations: {disabled}", flush=True)

    env = gym.make(args_cli.task, cfg=env_cfg)
    env = GraspAssistWrapperV2(env, target_key="object")   # SAME contract as training
    ...
    print(f"[eval-ppo-v2] loaded {args_cli.ppo_ckpt}  (grasp-assist v2 + B2 attach)", flush=True)
        ...
        with torch.inference_mode():
            ...
            if done.any():
                ...
                for i in done.nonzero(as_tuple=False).squeeze(-1).cpu().tolist():
                    ...
                    print(f"[eval-ppo-v2] ep {len(saved)}/{args_cli.num_episodes}  "
    ...
    summary = {
        "policy": "ppo_v2", "checkpoint": str(args_cli.ppo_ckpt), "task": args_cli.task,
    ...
    print(f"\n[eval-ppo-v2] LANDED: {summary['landed_rate']*100:.0f}% ({landed}/{n}), "
