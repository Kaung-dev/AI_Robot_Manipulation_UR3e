...
#
# Usage:
#   ./launch_teleop.sh [device] [dataset_file]
#
#   device:       handtracking | keyboard  (default: handtracking)
#   dataset_file: path to output HDF5      (default: ~/datasets/franka_pegboard_<timestamp>.hdf5)
#
# Examples:
#   ./launch_teleop.sh
#   ./launch_teleop.sh handtracking
#   ./launch_teleop.sh keyboard ~/datasets/test.hdf5

set -euo pipefail

ILAB="/home/steph/isaac-sim/isaac-sim-standalone-5.1.0-linux-x86_64/IsaacLab"
DEVICE="${1:-handtracking}"
TASK="Isaac-Lift-Pegboard-Franka-IK-Rel-Visuomotor-v0"

DATASET_DIR="$HOME/datasets"
mkdir -p "$DATASET_DIR"
DATASET_FILE="${2:-$DATASET_DIR/franka_pegboard_$(date +%Y%m%d_%H%M%S).hdf5}"
